...
def get_cg_repr(
    structure,
    mask_region: Optional[list] = None,
):
    ...
    if mask_region is not None:
        manual_mask_expand = np.random.rand(len(atom_to_token_index_)) > (mask_region[1] - mask_region[0]) / len(residue_index)
        manual_mask_expand = manual_mask_expand.astype(float)
        # manual_mask = np.ones(len(residue_index), dtype=float)
        # manual_mask[mask_region[0]:mask_region[1]] = 0
        # manual_mask_expand = np.array([manual_mask[i] for i in atom_to_token_index_])
    else:
        manual_mask_expand = np.ones(len(atom_to_token_index_), dtype=float)
    ...
    return {
        ...
        "atom_ca": np.concatenate(atom_ca) * manual_mask_expand[:, None],
        "atom_com": np.concatenate(atom_com) * manual_mask_expand[:, None],
        "ref_ca": np.concatenate(ref_ca) * manual_mask_expand[:, None],
        "ref_com": np.concatenate(ref_com) * manual_mask_expand[:, None],

        "calv_positions": np.concatenate(calv_positions) * manual_mask_expand[:, None],
...
def process_fn(
    ...
    if not mask:
        ...
        mask_region = [int(i) for i in accession_code.split(':')[-2:]]
        accession_code = accession_code.split(':')[0]
        print(accession_code)

    try:
        cg_repr = get_cg_repr(structure, mask_region=mask_region)
    ...
    if mask_region is None:
        ...
        accession_code_ = [f'{accession_code}_{mask_region[0]}_{mask_region[1]}']
        token_num = len(cg_repr["token_index"])
        moltype = np.unique(cg_repr["moltype"])
        with gzip.open(os.path.join(output_dir, f"{accession_code_[-1]}.pkl.gz"), "wb") as f:
            pickle.dump(cg_repr, f, protocol=pickle.HIGHEST_PROTOCOL)

        mask_length = mask_region[1] - mask_region[0]
        # shorten the mask from two end, get new data
        for shorten_length in range(mask_length // 2):
            new_mask_region = [mask_region[0] + shorten_length, mask_region[1] - shorten_length]
            cg_repr = get_cg_repr(structure, mask_region=new_mask_region)
            accession_code_.append(f'{accession_code}_{new_mask_region[0]}_{new_mask_region[1]}')
            with gzip.open(os.path.join(output_dir, f"{accession_code_[-1]}.pkl.gz"), "wb") as f:
                pickle.dump(cg_repr, f, protocol=pickle.HIGHEST_PROTOCOL)

        metadata = {
            "accession_code": accession_code_,
            "token_num": [token_num] * len(accession_code_),
            "moltype": [moltype] * len(accession_code_),
        }
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mask", "-m", default=False, action="store_true", help="Whether to mask a region")
    ...
    print(f"Using {cpu_num} CPUs for processing.")
    if cpu_num > 1:
    ...
    for result in results:
        if result[1] is not None:
            ...
            metadata["accession_code"].extend(result[0]["accession_code"])
            metadata["token_num"].extend(result[0]["token_num"])
            metadata["moltype"].extend(result[0]["moltype"])
