...
class BioFeatureTransform:
    ...
    def update_ref_features(data_object, translation_scale=1., training=True, repr="COM"):
        ...
        translation_mask = torch.ones(ref_positions.shape[0])
        if training:
            ...
            translation_mask_ = (torch.rand(data_object['token_index'].shape[0]) < 0.7).float()
            translation_mask_expand = [translation_mask_[i] for i in data_object['atom_to_token_index']]
