...
def residue_to_isrna1(residue):
    ...
    for atm_group in isrna1_base_topology[residue.res_name[0]].values():
        ...
        for atm in atm_group:
            atm_weight = rc.WEIGHT_MAPPING[atm[0]]
            ...
            if line.startswith("RESI"):
                ...
                for i_atm, atmName in enumerate(atmName_s):
                    if "_" in atmName:
                        ...
                        atmWeight = rc.WEIGHT_MAPPING[atmName[0]]
                        weight = weight.split("/")
                        weight = float(weight[0]) / float(weight[1]) * atmWeight
                    else:
                        weight = rc.WEIGHT_MAPPING[atmName[0]]
...
martini_path = os.path.abspath(os.path.join(abs_path, '..', 'common', 'martini3.top'))
top_s, weight_s = read_martini_topology(martini_path)
